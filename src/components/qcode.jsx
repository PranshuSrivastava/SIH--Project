...
function Qcode() {
  ...
  useEffect(() => {
    ...
    axios.get(`${url}/upload/question/${params.id}/${params.document}/${params.username}`)
      .then((result) => {
  ...
  useEffect(() => {
    ...
    axios.get(`${url}/upload/answer/${params.id}/${params.document}/${params.username}`)
      .then((result) => {
  ...
  async function SendAnswer(id, doc_name, index) {
    ...
    console.log("djbdbjdbj")


    const result = await axios.post(`${url}/upload/addanswer`, { title: teditor, content: ceditor, id: params.id, document: doc_name, index: index, ansuser: decoded.data.username, links: availablelinks, imglinks: questionimg })
  ...
  function Votes(vote_id, document, index, ansvote, ansindex, event) {
    ...
    axios.post(`${url}/upload/vote`, { id: params.id, document: document, ansindex: ansindex })
  ...
  async function saveinput(doc_name, event, index, comment) {
    ...
    const result = await axios.post(`${url}/upload/comment`, { comment: val, useronline: decoded.data.username, id: params.id, document: doc_name })
    ...
    useEffect(() => {
      ...
      async function extra() {
        if (c <= 1) {
          ...
          const r = await axios.post("https://cyduck2.herokuapp.com/upload/autodelete", { document: props.d, index: props.idx })
  ...
  async function Upvote(document, index, ansindex, state, upvotevalue, id, event) {
    ...
    const res = await axios.post(`${url}/upload/upvote/${state}`, { id: params.id, document: document, ansindex: ansindex, username: decoded.data.username })
  ...
  async function EditCode(event) {
    ...
    const r = await axios.post(`${url}/upload/codeeditor`, { document: params.document, id: params.id, content: givencode })
